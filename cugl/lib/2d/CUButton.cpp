...
bool Button::initWithData(const SceneLoader* loader, const std::shared_ptr<JsonValue> data) {
    ...
    std::shared_ptr<JsonValue> upnode = data->get("up");
    Size size;
    if (upnode == nullptr) {
        ...
        _upnode  = loader->build("up",upnode);
        _upcolor = _upnode->getColor();
        _upform  = upnode->get("layout");
        size = _upnode->getSize();
    }

    _upnode->setAnchor(Vec2::ANCHOR_CENTER);
    std::shared_ptr<JsonValue> downnode = data->get("down");
    if (downnode == nullptr) {
        _downnode  = nullptr;
        _downcolor = _upcolor*Color4::GRAY;
    } else if (downnode->get(0)->key() == "") {
        _downnode = nullptr;
        ...
        _downnode = loader->build("down",downnode);
        _downcolor = _downnode->getColor();
        _downform  = downnode->get("layout");
...
void Button::doLayout() {
    // TODO: This design needs a complete rethink for 2019
    // We need this since these nodes were added at the wrong time for layout.
    if (_upform && _layout) {
        _layout->add(_upnode->getName(), _upform);
        _upform = nullptr;
    }
    if (_downform && _layout) {
        _layout->add(_downnode->getName(), _downform);
        _downform = nullptr;
    }
    Size osize = _upnode->getSize();
    Node::doLayout();
    
    if (_bounds.getType() == Poly2::Type::SOLID) {
        Size nsize = _upnode->getSize();
        Vec2 scale;
        scale.x = (osize.width > 0 ? nsize.width/osize.width : 0);
        scale.y = (osize.height > 0 ? nsize.height/osize.height : 0);
        _bounds *= scale;
    }
}
